    ...
    function validateSync(observable, rule, ctx) {
        ...
        if (!rule.validator(observable(), ctx.params === undefined ? true : ctx.params)) { // default param is true, eg. required = true
            ...
            if(configuration.enableErrorDetails) {
                ...
                observable.errorDetails.message(exports.formatMessage(ctx.message || rule.message, ctx.params));
            }
            observable.__valid__(false);
            return false;
        } else {
        ...
        var callBack = function (valObj) {
            ...
            if (!isValid) {
                ...
                if(configuration.enableErrorDetails) {
                    observable.errorDetails.rule(rule);
                    observable.errorDetails.params(ctx.params);
                    observable.errorDetails.message(exports.formatMessage(msg || ctx.message || rule.message, ctx.params));
                }
